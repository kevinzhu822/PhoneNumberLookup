...
-  **Extra parameters in the query string should be ignored.** For instance, if a phoneNumber is provided with a valid country code included within it, but an additional `countCode` parameter is provided, the API would ignore the `countCode` parameter and handle `phoneNumber` as normal.

-  **If countryCode is provided, it should not be ignored, even if ignoring it would result in a valid response.** This scenario comes into play when a valid `phoneNumber` such as `+16573310806` is provided, but an invalid `countryCode` such as `USA` is also provided. If we just ignore the `countryCode`, this would result in a successful response. However, I assume that if a countryCode were provided, it is important and we should not ignore it.

-  **A countryCode prepended to a phoneNumber could potentially result in a valid phoneNumber.** This is the rationale behind the "Missing Input Error" response type, where a `phoneNumber` is identified as invalid but a `countryCode` was not provided. This is because if a `countryCode` were provided, there is a possibility that the `phoneNumber` could have become valid. For example, if the `phoneNumber` "6573310806" were provided without a `countryCode`, it would be considered invalid. However, if the `countryCode` : `"US"` were provided, we would prepend `+1` to `phoneNumber`, transforming it into a valid I.164 phone number.

-  **Even though Canada and the US share a country code (+1), they should be treated as separate countries.** Under this assumption, scenarios where a valid Canadian phone number is provided such as `6047628322` but the `countryCode` provided is `US`, an error response should be returned even though `+16046728322` is a valid Canadian phone number. Because the provided country is not Canada (`CA`), we should error on the basis on data inconsistency.
